...
window.firebaseReady = new Promise((resolve) => {
  ...
  let loaded = 0;
  scripts.forEach((src) => { const script = document.createElement("script"); script.src = src; script.onload = () => { loaded += 1; if (loaded === scripts.length) { firebase.initializeApp(FIREBASE_CONFIG); window.firebaseClient = firestoreAdapter(firebase); resolve(window.firebaseClient); } }; script.onerror = () => { window.firebaseMode = "demo"; window.firebaseClient = demoAdapter(); resolve(window.firebaseClient); }; document.head.appendChild(script); });
});
