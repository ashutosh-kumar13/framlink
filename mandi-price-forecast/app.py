...
app = Flask(__name__)
# Cleanest CORS setup: allows everything for local development
CORS(app)
...
CACHE_TTL = 3600 * 24
DISCOVERY_CACHE_TTL = 3600 * 6

# Daily Resource for Fallback
DAILY_RESOURCE_ID = "9ef84268-d588-465a-a308-a864a43d0070"

# --- Global Caches & Registry ---
market_cache = {
    "major_crops": {},
    "trained_models": {}
}
training_registry = {} # key: selection_hash -> "pending" | "ready" | "error"

def get_selection_hash(state, district, market, commodity):
    return f"{state}_{district}_{market}_{commodity}".lower().replace(" ", "_")

def background_train_task(state, district, market, commodity, paths):
    """Deep 3-year analysis + weather integration in background."""
    h = get_selection_hash(state, district, market, commodity)
    try:
        print(f"\n[BACKGROUND] Starting deep analysis for {commodity}...")
        # Fetch deep 3-year history + weather
        success, msg = fetch_mandi_data(state, district, market, commodity,
                                        target_records=2000,
                                        save_path=paths["raw"],
                                        fetch_weather=True)
        if success:
            clean_mandi_data(state, district, market, commodity, raw_path=paths["raw"], clean_path=paths["clean"])
            train_final_model(model_path=paths["model"], clean_path=paths["clean"], search_level="Mandi Match (Deep)")
            training_registry[h] = "ready"
            print(f"[BACKGROUND] AI Model Ready for {commodity}.")
        else:
            training_registry[h] = "error"
